...
--
-- Example usage:
-- let colorImage = createImage 200 colorGrad
-- writeImage "../images/color.png" colorImage

-- normalizes pixel input [0, dim]^2 -> [-1, 1]^2
mapOnto :: Int -> (Int, Int) -> (Double, Double)
mapOnto dim (x, y) = (
  ((fromIntegral x) - halfDim) / halfDim,
  ((fromIntegral y) - halfDim) / halfDim)
...
colorGrad (x, y)
  ...
  | y == 0 = (0.0, 0.0, 0.0) -- prevent divide by zero
...
createImage dim func = makeImageR VU (dim, dim) (mkPixel . func . mapOnto dim)
